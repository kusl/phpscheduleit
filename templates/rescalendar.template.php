...
* @param int $starttime starting time of the day for this reservation's schedule
* @param int $endtime ending time of the day for this reservation's schedule
* @param int $timespan the time span interval for this reservation's schedule
* @param bool $is_private if we are in privacy mode and should hide user details
*/
function print_day_resource_reservations($reservations, $datestamp, $days, $scheduleid, $starttime, $endtime, $timespan, $is_private = false) {
	...
	for ($i = 0; $i < count($reservations); $i++) {	
		...
		$starttime = ($reservations[$i]['start_date'] >= $datestamp) ? ($reservations[$i]['starttime']) : $starttime;
		$endtime = ($reservations[$i]['end_date'] <= $datestamps[$days-1]) ? ($reservations[$i]['endtime']) : $endtime;
		...
		if ($day != $endday) {
			for ($d = $day+1; $d <= $endday; $d++) {
				if ($datestamps[$d] == $reservations[$i]['end_date']) {
					...
					if ($reservations[$i]['endtime'] > $starttime) {
						$hour_line[$starttime][$d] = &$reservations[$i];
					}	
				}
				else {
					$hour_line[$starttime][$d] = &$reservations[$i];
	...
	for ($time = $starttime; $time < $endtime; $time += $timespan) {
		...
		for ($date = 0; $date < $days; $date++) {
			if (isset($hour_line[$time][$date])) {
				...
				$rowspan = intval(($endtime - $starttime)/$timespan);
...
* @param bool $is_private if we are in privacy mode and should hide user details
*/
function print_signup_sheet($reservations, $datestamp, $days, $starttime, $endtime, $timespan, $resource_name, $is_private = false) {
